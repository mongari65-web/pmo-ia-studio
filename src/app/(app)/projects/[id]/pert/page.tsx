...
export default function PertPage() {
  ...
  async function loadData() {
    ...
    const { data: pertDocs } = await supabase.from('documents').select('content,updated_at').eq('project_id', id).eq('doc_type', 'pert_data').order('created_at', { ascending: false }).limit(1)
